...
function QuickActions({ className, compact }: { className?: string; compact?: boolean }) {
  ...
  const actions = [
    { label: "Nuevo gasto", icon: ArrowUpRight, tone: "text-[#b4482c] bg-[#e0694a]/10", run: () => openTransaction({ kind: "expense" }) },
    { label: "Nuevo ingreso", icon: ArrowDownLeft, tone: "text-teal-700 bg-teal-50", run: () => openTransaction({ kind: "income" }) },
    { label: "Transferencia", icon: ArrowLeftRight, tone: "text-navy-700 bg-navy-900/5", run: () => openTransaction({ kind: "transfer" }) },
    { label: "Nuevo pago programado", icon: CalendarClock, tone: "text-navy-700 bg-navy-900/5", run: () => openPlanned({ kind: "expense" }) },
...
export function AppShell({ name, todayLabel, children }: { name: string; todayLabel: string; children: React.ReactNode }) {
