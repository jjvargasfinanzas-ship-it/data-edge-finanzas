  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-[1.3fr_1fr]">
        <Field label="Concepto" htmlFor="name" error={fe.name}>
          <Input
            id="name"
            name="name"
            required
            ...
            data-autofocus=""
            placeholder={kind === "income" ? "Ej. Salario, honorarios, arriendo local" : kind === "transfer" ? "Ej. Pago tarjeta Visa" : "Ej. Arriendo apartamento"}
          />
        </Field>
        <Field label={kind === "income" ? "Valor que esperas recibir" : "Valor"} htmlFor="amount" error={fe.amount}>
      ...
      {kind !== "transfer" && (
        <Field label="Categoría">
          <CategoryPicker
            ...
            onChange={(id) => {
              setCategoryId(id);
              // Si no ha escrito un concepto, se propone el de la categoría
              if (!nameTouched) setName(categories.find((c) => c.id === id)?.name ?? "");
            }}
          />
        </Field>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Field
          label={frequency === "once" ? "Fecha" : "Fecha de inicio"}
          htmlFor="start_date"
          error={fe.start_date}
          hint={frequency === "semimonthly" ? "Se repite el 15 y el último día." : "Fechas anteriores a hoy no quedan como pendientes."}
        >
          <Input id="start_date" name="start_date" type="date" required defaultValue={initial.start_date ?? today} />
        </Field>
        {frequency !== "once" && (
          <Field label="Fecha de finalización" htmlFor="end_date" error={fe.end_date} hint="Opcional. Vacío = sin fin.">
