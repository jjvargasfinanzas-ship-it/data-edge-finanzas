...
export function StatTile({ label, value, currency, hint, icon, tone, href, signed, hintAccent }: StatTileProps) {
  const t = tone ?? (value < 0 ? "negative" : "neutral");
  const body = (
    <>
      <div className="flex items-start gap-2">
        {icon && (
          <span className="-my-1 grid size-7 shrink-0 place-items-center rounded-lg bg-tint text-teal-700 max-[380px]:hidden [&_svg]:size-3.5" aria-hidden>
            {icon}
          </span>
        )}
        <p className="line-clamp-2 min-w-0 flex-1 text-xs leading-snug font-medium text-muted">{label}</p>
        {href && <ChevronRight className="mt-0.5 size-3.5 shrink-0 text-muted/70 transition group-hover:translate-x-0.5 group-hover:text-teal-700" aria-hidden />}
      </div>
      <div className="@container mt-auto pt-2.5">
        <FitMoney value={value} currency={currency} signed={signed} className={valueTone[t]} />
        {hint && <p className={cn("mt-0.5 truncate text-[11px]", hintAccent ? "font-semibold text-teal-700" : "text-muted")}>{hint}</p>}
      </div>
    </>
  );
  const cls = "card group flex h-full min-h-[104px] flex-col p-3.5 sm:p-4";
  return href ? (
