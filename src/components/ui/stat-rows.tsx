    ...
    <dl className={cn("divide-y divide-line", className)}>
      {rows.map((r, i) => (
        <div key={i} className={cn("flex items-center gap-3 py-2.5", r.tone === "total" && "pt-3")}>
          {r.op && (
            <span className="grid size-6 shrink-0 place-items-center rounded-full bg-canvas text-xs font-bold text-muted" aria-hidden>
              {r.op}
            </span>
          )}
          <dt className="min-w-0 flex-1">
            <span className={cn("block text-sm", r.tone === "total" ? "font-bold text-ink" : "text-ink-2")}>{r.label}</span>
            {r.hint && <span className="block text-xs text-muted">{r.hint}</span>}
          </dt>
          <dd
            className={cn(
              "num shrink-0 text-right whitespace-nowrap",
              r.tone === "total" ? "text-lg font-bold text-ink" : "text-[15px] font-semibold",
