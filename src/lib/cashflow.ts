 ...
 * - Programados vencidos (últimos días) sin registrar se muestran hoy como pendientes.
 */
...
export function expandPlanned(p: Params): Occurrence[] {
  const lookback = p.overdueLookbackDays ?? 15;
  const from = addDays(p.today, -lookback);
  const accounts = new Map(p.accounts.map((a) => [a.id, a]));
  const done = new Set(
    p.registered.filter((r) => r.planned_item_id && r.planned_date).map((r) => `${r.planned_item_id}|${r.planned_date}`),
  );
  ...
  for (const item of p.planned) {
    ...
    for (const d of occurrencesBetween(item, from, p.horizonEnd)) {
      if (done.has(`${item.id}|${d}`)) continue;
      const overdue = d < p.today;
      const base = convert(item.amount, acc.currency, p.baseCurrency, p.rates);
      let cashEffect = 0;
